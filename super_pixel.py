...
class SuperPixel:
    def __init__(self, monthly_traffic=17500.0, cpa=150.0, 
        ...
        self.sales = sales if sales is not None else self.get_sales()
        ...
        self.anon_traffic = anon_traffic if anon_traffic is not None else self.get_anon_traffic()
        self.idr_match = idr_match  # ID Resolution Match
        self.customer_matches = customer_matches if customer_matches is not None else self.get_customer_matches()
        self.verification = verification  # verification
        self.verified_mp = verified_mp if verified_mp is not None else self.get_verified_mp()
        self.re_opt_in = re_opt_in  # Re-opt-in
        self.recovered_leads = recovered_leads if recovered_leads is not None else self.get_recovered_leads()
        self.rsr = rsr  # reactivation sales rate
        self.recovered_sales = recovered_sales if recovered_sales is not None else self.get_recovered_sales()
        self.recovered_revenue = recovered_revenue if recovered_revenue is not None else self.get_recovered_revenue()
...
# Example usage:
sp = SuperPixel()
print(sp.get_recovered_revenue())
